...
def draw_boxes_after_no(img, bbox, identities=None, offset=(0,0)):
    ...
    if (cli_time == 20):
        ...
        t.start()
        # t.join()
        cv2.rectangle(img, (0,0), (1000,110), [255,255,255], -1)
        cv2.putText(img, "Client Missing! Listening...", (0,80),cv2.FONT_HERSHEY_SIMPLEX, 3, [0, 0, 0], 10)
        return draw_boxes_plain(img, bbox,identities, offset)
    if (cli_time > 20 and cli_time <110):
        cv2.rectangle(img, (0,0), (1000,110), [255,255,255], -1)
        cv2.putText(img, "Listening the word...", (0,80),cv2.FONT_HERSHEY_SIMPLEX, 3, [0, 0, 0], 10)
        string = que.get()
        if(string is 'RESTART TRACKING'):
            ...
            for i in range(1, 100):
                track[i] = 0
            cv2.rectangle(img, (0,0), (1000,110), [255,255,255], -1)
            cv2.putText(img, 'RESTART TRACKING ', (0,80), cv2.FONT_HERSHEY_SIMPLEX, 3, [0, 0, 0], 10)
            return draw_boxes_before(img, bbox, identities, offset)
        else:
            cv2.putText(img,'HEAR NOTHING ', (0,240),cv2.FONT_HERSHEY_SIMPLEX, 3, [0, 0, 0], 10)
            return draw_boxes_plain(img, bbox, identities, offset)
    if (cli_time >= 110):
        track_modes =1
        cli=0
...
def draw_boxes_plain(img, bbox, identities=None, offset=(0,0)):
    for i, box in enumerate(bbox):
        ...
        #drawrect(img,  (x1, y1), (x2, y2), color, 5)
        #drawrect(img, (x1, y1), (x1 + t_size[0] + 3, y1 + t_size[1] + 4), color, 1, 'dotted')
        cv2.rectangle(img, (x1, y1), (x2, y2), color, 10)
        if (x1-5<=0): x1=0
        cv2.rectangle(img, (x1-5, y1-20), (x1 + t_size[0]+250, y1 + t_size[1]), color, -1)
        track[id] = track[id] + 1
        cv2.putText(img,"Customer ID: " +label, (x1, y1 + t_size[1] -10), cv2.FONT_HERSHEY_PLAIN, 2, [255, 255, 255], 2)
    return img
...
def draw_boxes_before(img, bbox, identities=None, offset=(0,0)):
    ...
    if(track_modes==1):
        cv2.rectangle(img, (0,0), (1000,110), [255,255,255], -1)
        cv2.putText(img, "Detecting Client", (0,80), cv2.FONT_HERSHEY_SIMPLEX, 3, [0, 0, 0], 10)
    if(track_modes==3):
        cv2.rectangle(img, (0,0), (2000,110), [255,255,255], -1)
        cv2.putText(img, "Client Missing! Start Finding", (0,80),cv2.FONT_HERSHEY_SIMPLEX, 3, [0, 0, 0], 10)
    """
    for i in range(1, 100):
        if (track[i] >= 863):
            track_modes = 2
    ...
    for i, box in enumerate(bbox):
        ...
        #drawrect(img,  (x1, y1), (x2, y2), color, 10)
        #drawrect(img, (x1, y1), (x1 + t_size[0] + 3, y1 + t_size[1] + 4), color, 5)
        cv2.rectangle(img, (x1, y1), (x2, y2), color, 10)
        if (x1-5<=0): x1=0
        cv2.rectangle(img, (x1-5, y1-20), (x1 + t_size[0]+250, y1 + t_size[1]), color, -1)
        track[id] = track[id] + 1
...
def draw_boxes_after_yes(img, bbox, identities=None, offset=(0,0)):
    ...
    '''
        cv2.rectangle(img, (0,0), (1000,110), [255,255,255], -1)
        cv2.putText(img, "Following Client No."+str(cli),  (0,80), cv2.FONT_HERSHEY_SIMPLEX, 3, [0, 0, 0], 10)
    if(track_modes==3):
        cv2.rectangle(img, (0,0), (1000,110), [255,255,255], -1)
        cv2.putText(img, "Client Missing! Start Finding " + str(cli_time), (0,80),cv2.FONT_HERSHEY_SIMPLEX, 3, [0, 0, 0], 10)
        '''
    for i, box in enumerate(bbox):
        x1, y1, x2, y2 = [int(i) for i in box]
        x1 += offset[0]
        ...
        #drawrect(img,  (x1, y1), (x2, y2), color, 10)
        #drawrect(img, (x1, y1), (x1 + t_size[0] + 3, y1 + t_size[1] + 4), color, 10, 'dotted')
        if(id!=cli):
        ...
        track[id] = track[id] + 1
        if (id == cli):
            drawrect(img, (x1, y1), (x2, y2), [0,0,255], 10)
            #drawrect(img, (x1, y1), (x1 + t_size[0] + 3, y1 + t_size[1] + 4), color, 5, 'dotted')
            #cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 255), 6)
            if (x1-5<=0): x1=0
            cv2.rectangle(img, (x1-5, y1-20), (x1 + t_size[0]+250, y1 + t_size[1]), [0,0,255], -1)
            cv2.putText(img, "CLIENT No."+str(cli), (x1, y1 + t_size[1] -10), cv2.FONT_HERSHEY_PLAIN, 2, [255, 255, 255], 2)
